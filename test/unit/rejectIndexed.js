...
var rejectIndexed = requireR('rejectIndexed');
...
describe('rejectIndexed', function() {
  ...
  it('works just like a normal reject', function() {
    assert.deepEqual(rejectIndexed(even, [1, 2, 3, 4, 5]), [1, 3, 5]);
  });

  it('passes the index as a second parameter to the predicate', function() {
    assert.deepEqual(rejectIndexed(everyOther, [8, 6, 7, 5, 3, 0, 9]), [6, 5, 0]);
  });

  it('passes the entire list as a third parameter to the predicate', function() {
    assert.deepEqual(rejectIndexed(lastTwo, [8, 6, 7, 5, 3, 0, 9]), [8, 6, 7, 5, 3]);
  });

  it('returns an empty array if no element matches', function() {
    assert.deepEqual(rejectIndexed(function(x) { return x < 100; }, [1, 9, 99]), []);
  });

  it('returns an empty array if asked to filter an empty array', function() {
    assert.deepEqual(rejectIndexed(function(x) { return x > 100; }, []), []);
  });

  it('is curried', function() {
    var everyOtherPosition = rejectIndexed(everyOther);
