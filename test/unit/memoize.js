...
var memoize = requireR('memoize');


describe('memoize', function() {
  it('calculates the value for a given input only once', function() {
    var ctr = 0;
    var fib = memoize(function(n) {ctr += 1; return n < 2 ? n : fib(n - 2) + fib(n - 1);});
    var result = fib(10);
  ...
  it('does not rely on reported arity', function() {
    var identity = memoize(function() { return arguments[0]; });
  ...
  it('can be applied to function with optional arguments', function() {
    var count = 0;
    var f = memoize(function concat(a, b) {
      count += 1;
  ...
  it('differentiates values with same string representation', function() {
    var f = memoize(function(x) { return x; });
