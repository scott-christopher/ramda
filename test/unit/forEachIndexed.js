...
var forEachIndexed = requireR('forEachIndexed');
...
describe('forEachIndexed', function() {
  ...
  it('performs the passed in function on each element of the list and passes in the index and list as well', function() {
    var sideEffect = {};
    forEachIndexed(function(elem, idx) { sideEffect[elem.x] = idx; }, list);
    assert.deepEqual(sideEffect, {1: 0, 100: 1, 300: 2, 234: 3});
  });

  it('returns the original list', function() {
    var s = '';
    assert.deepEqual(forEachIndexed(function(obj) { s += obj.x; }, list), list);
    assert.strictEqual('1100300234', s);
  });

  it('handles empty list', function() {
    assert.deepEqual(forEachIndexed(function(x, idx) { return x + idx; }, []), []);
  });

  it('is curried', function() {
    var sum = 0;
    var xe = forEachIndexed(function(x, idx) { sum += (x + idx); });
