...
describe('none', function() {
  ...
  it('returns true if no elements satisfy the predicate', function() {
    assert.strictEqual(none(even, [1, 3, 5, 7, 9, 11]), true);
  });

  it('returns false if any element satisfies the predicate', function() {
    assert.strictEqual(none(even, [1, 3, 5, 7, 8, 11]), false);
  });

  it('returns true for an empty list', function() {
    assert.strictEqual(none(T, []), true);
  });
  ...
  it('works with more complex objects', function() {
    ...
    assert.strictEqual(none(len3, xs), true);
    assert.strictEqual(none(hasA, xs), false);
  });

  it('is curried', function() {
    assert.strictEqual(none(even)([1, 3, 5, 6, 7, 9]), false);
  });
});
