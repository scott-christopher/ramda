...
describe('forEach', function() {
  ...
  it('performs the passed in function on each element of the list', function() {
    var sideEffect = {};
    forEach(function(elem) { sideEffect[elem.x] = elem.y; }, list);
    assert.deepEqual(sideEffect, {1: 2, 100: 200, 300: 400, 234: 345});
  });

  it('returns the original list', function() {
    var s = '';
    assert.deepEqual(forEach(function(obj) { s += obj.x; }, list), list);
    assert.strictEqual('1100300234', s);
  });

  it('handles empty list', function() {
    assert.deepEqual(forEach(function(x) { return x * x; }, []), []);
  });

  it('is curried', function() {
    var xStr = '';
    var xe = forEach(function(x) { xStr += (x + ' '); });
